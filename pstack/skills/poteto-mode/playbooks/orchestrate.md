...
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, with the full spawn schema available at each level). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Workers run on this machine and can reach anything it can: `control-ui` or `control-cli` runtime verification (from `cursor-team-kit`), local transcripts under `~/.claude/projects/<slug>/`, simulators and local IDE state, and auth that exists only here. Point briefs at repo paths rather than inlining context, to keep each worker's window free for the work. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model from its worker.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Every restack runs on this machine, so keep stacks shallow and serialize restacks within a stack; a deep restack at this scale takes the laptop down.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness.
...
- After a restart every in-flight agent is dead, but the work they pushed is not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach landed work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. Because nothing survives the restart, push early and often: an unpushed worktree is the only work a restart can actually lose. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
