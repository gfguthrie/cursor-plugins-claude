...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under this workspace's `~/.claude/projects/<slug>/` directory (`<slug>` is the workspace path with each `/`, `.`, and `_` turned into `-`). Don't glob across `~/.claude/projects/*/`; that reads unrelated private chats. Walk the log against what actually happened:
