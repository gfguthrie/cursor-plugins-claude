...
Transcripts live at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the workspace path with each "/", "." and "_" turned into "-" (so `/Users/you/proj` becomes `-Users-you-proj`, keeping the leading dash). Subagent transcripts sit beside them under `<session-uuid>/subagents/agent-*.jsonl`. Every line is one chat message.
